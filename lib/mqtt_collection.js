...
Mongo.Collection.prototype.mqttConnect = function(uri, topics, options, mqttOptions) {
	...
	this.find().observe({
		...
		changed: function(newDoc, oldDoc) {
			if(!init) {
				if(newDoc && newDoc.broadcast && self._mqttClient) {
					var msg = typeof newDoc.message === 'object' ? JSON.stringify(newDoc.message) : newDoc.message + "";
					self.remove({ _id: newDoc._id });
					self._mqttClient.publish(newDoc.topic, msg);
				}
			}
		}
	});
